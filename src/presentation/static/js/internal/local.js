...
function getFileType(contentType) {
	if (!contentType || contentType === "unknown")
		return '<span class="status-badge discovery">Unknown</span>';
...
function getEmbedStatusBadge(embedStatus) {
	...
	const config = statusMap[embedStatus] || {
		class: "discovery",
		label: embedStatus || "Unknown",
	};
...
async function loadLocalFiles() {
	try {
		const response = await fetch(
			"/api/v1/files/local/list?limit=1000&offset=0",
		);
		const data = await response.json();

		if (!data.success || !data.data) {
			localFilesBody.innerHTML =
				'<tr><td colspan="5" class="empty-state">No local files found.</td></tr>';
		...
		pendingCount.textContent = files.filter(
			(f) => f.embed_status === "discovery",
		).length;
		embeddedCount.textContent = files.filter(
			(f) => f.embed_status === "embedded",
		).length;

		if (files.length === 0) {
			localFilesBody.innerHTML =
				'<tr><td colspan="5" class="empty-state">No local files found. Upload a file to get started.</td></tr>';
			return;
		}

		localFilesBody.innerHTML = files
			.map(
				(file, idx) => `
			<tr>
		...
		localFilesBody.innerHTML =
			'<tr><td colspan="5" class="empty-state">Error loading files.</td></tr>';
	}
}
...
async function deleteFile(idx, originalUrl) {
	...
	try {
		const response = await fetch(
			`/api/v1/files/local/delete?file_path=${encodeURIComponent(originalUrl)}`,
		);
		const data = await response.json();
